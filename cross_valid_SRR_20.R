...
cross_valid <- function(dataset, runs) {
  ...
  for(i in 1:runs){
    ...
    fit_strucchange <- lm(R_1 ~ SSB_lag*(SSB_lag < best_brk[1]) + SSB_lag*(SSB_lag > best_brk[1]), data = test_data)
    fit_strucc1 <- fitted(strucc_cod)
...
rmse_mean_plaice <- cross_valid(dataset = dat, runs = 20)

#ALEX:
#linear.model beverton   ricker   segmented segmented.log  segmented.best.glm
# 601925.3    608782.5   612596.8  610958.5       1317100            1317100
...
rmse_mean_cod <- cross_valid(dataset = dat, runs = 20)
# linear.model  beverton   ricker     segmented     segmented.log   segmented.best.glm
# 
...
#set up rmse dataframe
rmse_values_haddock <- data.frame("run" = c(1:20),
                                  "linear model" = c(1:20),
                                  "beverton"= c(1:20),
                                  "ricker"= c(1:20),
                                  "segmented"= c(1:20),
                                  "segmented log"= c(1:20),
                                  "segmented best glm"= c(1:20))
...
# linear.model  beverton   ricker     segmented     segmented.log     segmented.best.glm
# 271948.8      10.5       10.5      10.5          10.5               10.5
...
names(dat) <- c("Year", "R", "SSB_lag", "R_log", "SSB_lag_log")
rmse_mean_herring <- cross_valid(dataset = dat, runs = 20)
# linear.model beverton   ricker      segmented     segmented.log   segmented.best.glm
# 13931610     13070202    13348700  12047692      31685137           31685137
...
#set up rmse dataframe
rmse_values_hake <- data.frame("run" = c(1:20),
                               "linear model" = c(1:20),
                               "beverton"= c(1:20),
                               "ricker"= c(1:20),
                               "segmented"= c(1:20),
...
# linear.model    beverton          ricker    segmented     segmented.log   segmented.best.glm
# 122106.9        10.5 123301.5  121031.7      360356.3           360356.2
...
#set up rmse dataframe
rmse_values_saithe <- data.frame("run" = c(1:20),
                                 "linear model" = c(1:20),
                                 "beverton"= c(1:20),
                                 "ricker"= c(1:20),
                                 "segmented"= c(1:20),
...
# linear.model   beverton        ricker     segmented     segmented.log   segmented.best.glm
# 28770.74        10.5 27571.08  26693.61      47616.01           47615.98  